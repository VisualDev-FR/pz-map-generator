...
// md5::toHash
// Computes a string MD5 hash from a bytes buffer.
std::string MD5::toHash(const std::vector<uint8_t>& data)
{
    MD5 alg;

    if (!data.empty())
        alg.Update((unsigned char*)data.data(), (unsigned int)data.size());
